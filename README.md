...
Thanks for the meeting. We have established that the background of your vision of this system is to go from user prompt > architecture diagram > terraform codes. Two main benefits from such automation:
1. Ensure projects stick to their approved architecture and not to use cloud resources that are outside of this approved architecture.
2. Standardise the review and enforcement process from architecture to cloud.

There are several challenges for this vision, and with an innovation risk management perspective, we ought to tackle these one at a time as individual experiments. The use of AI in this scenario is still in its infancy, and there are no proven off-the-shelf solutions.

We can broadly separate into 4 experimental projects, as below. We can do Projects 1 and 3 in parallel.
1.	Project 1: Prompt to Architecture Diagram. The main objective of this project is to find/create a reliable AI tool that can create architecture diagrams. Since SCB is also into Azure, do we want to be able to create architecture diagrams for both AWS and Azure?
2.	Project 2: Architecture Diagram Improvement. The main objective of this project is to improve upon the AI tool from Project 1 so that it can create architecture diagrams that meets the standards of SCB.
3.	Project 3: Architecture Diagram to Terraform code. The main objective of this project is to find/create a reliable AI tool that can create and edit Terraform codes based on Architecture diagrams from project 2.
4.	Project 4: Terraform Deploy. The main objective of this project is to ascertain the quality of the Terraform codes created in Project 3, and to find ways to improve the AI tool that generate the Terraform codes to acceptable level for deployment into SCB env.

Please feel free to comment on the proposed experimental projects. We will need to discuss about funding these experiments in subsequent meeting.

From: AM,Venkatesan <Venkatesan.AM@sc.com> 
Sent: Monday, January 12, 2026 5:18 PM
To: Neumann, Thorsten <Thorsten.Neumann@sc.com>; cdo-ai-r&e <cdo-ai-r&e@exchange.standardchartered.com>; Chew, Ern <Ern.Chew@sc.com>
Subject: RE: CDO AI Research & Experimentation (R&E) - Use Case Template for "AI-powered Infrastructure as Code (AIaC) - Application Architecture and Terraform code generation for AWS or Azure"

INTERNAL

Hi Thorsten,
Thanks for the update. Already I have analyzed suitable AI solution in the market for this use case. I want to explore only SCB controlled AWS Or Azure cloud models or OpenAI models with some code development in python.

I am planning to use sample data or Internal data during POC and development phase. In SIT phase, planning to use any AWS or Azure on-boarded application architecture to ensure SIT outcomes matching with SCB standard.

I need your team guidance in initial analysis and plan the POC.

AI Solution in Market:

Brainboard - Able to generate AWS application architecture diagram and the corresponding Terraform code from a natural language prompt in Brainboard 
using its "Create with AI" feature.
CloudSketcher - Able to generate both AWS application architecture diagrams and the corresponding Terraform code from a natural language prompt  
Miro AI - Able to generate AWS Application architecture from prompt. This functionality is handled by its built-in features, SketchDraw and SketchTF, respectively. 
DiagramGPT - Able to generate AWS Application architecture from prompt


AWS-Specific Solution :
AWS offers a command-line interface tool that uses generative AI to create architecture diagrams following AWS best practices: 
Kiro CLI with AWS Diagram MCP: This solution, mentioned in an AWS blog, uses Kiro CLI along with a specific Model Context Protocol (MCP) server to interpret natural language prompts. It generates diagrams by producing and executing Python code using the diagrams package, which has access to the official AWS icon set.

The Kiro CLI is an AI-powered command-line interface developed by AWS that can generate Azure application architecture, although its native integrations are primarily with AWS services. It uses natural language prompts and the Model Context Protocol (MCP) to interact with external tools and data sources. 
...
Thanks, received. Added @Chew, Ern to be SPOC on this exploration; he himself familiar with Terraform and DevOps.  

I would think there are quite a few open-source efforts for this purpose: 
•	SketchTF: Transforming Cloud Architecture Diagram to Terraform Modules | by Amoghavarsh | Medium
•	Building a Terraform Module with GenAI + MCP | Gourav J. Shah posted on the topic | LinkedIn
•	Generating Terraform code with GenAI tools — AWS Bedrock | by Guillermo Contreras | Medium

Most of these are external to bank opens-source software and/or variety of models.  It might help to do some initial PoCs.

Prior to using AI Factory for LLMs, perhaps our AI R&E AWS environment might be better start?

Or do you plan to work with Internal / Confidential data ?
...
From: AM,Venkatesan <Venkatesan.AM@sc.com> 
Sent: Friday, January 9, 2026 4:50 PM
To: cdo-ai-r&e <cdo-ai-r&e@exchange.standardchartered.com>
Subject: CDO AI Research & Experimentation (R&E) - Use Case Template for "AI-powered Infrastructure as Code (AIaC) - Application Architecture and Terraform code generation for AWS or Azure"

INTERNAL

Hi CDO AI – R&E Team, 
 PFA attached Use case – Business benefit template for “AI-powered Infrastructure as Code (AIaC) - Application Architecture and Terraform code generation for AWS or Azure".

Kindly review and provide inputs to plan this POC.
